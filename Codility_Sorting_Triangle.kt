//Triangle Task description ; An array A consisting of N integers is given. A triplet (P, Q, R) is triangular if 0 ≤ P < Q < R < N and:
...
//  A[0] = 10    A[1] = 50    A[2] = 5
//  A[3] = 1 the function should return 0. 
//Constraints ; N is an integer within the range [0..100,000]; //each element of array A is an integer within the range [−2,147,483,648..2,147,483,647].
    ...
    for(i in A.indices){
        for(j in i+1..A.size-1){
            for(k in j+1..A.size-1){
                if(isTrinangle(A[i],A[j],A[k])) return 1
